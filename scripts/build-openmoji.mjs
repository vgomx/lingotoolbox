...
import { WHOLE_GROUPS, EXTRA, FLAGS } from './openmoji-selection.mjs';
...
// Flags ship as files and as their own list, but never as illustrations — see
// the note beside FLAGS in openmoji-selection.mjs.
const flagRecords = records.filter((r) => flagHexes.has(r.hex));
const illustrationRecords = records.filter((r) => !flagHexes.has(r.hex));

const licence = await (await fetch(`${CDN}/LICENSE.txt`)).text();
await writeFile(path.join(outDir, 'LICENSE.txt'), licence);

const usedGroups = GROUPS.filter((g) => illustrationRecords.some((r) => r.group === g.id));
...
export const ILLUSTRATIONS: Illustration[] = ${JSON.stringify(
  illustrationRecords.map(({ hex, file, name, group, keywords }) => ({ hex, file, name, group, keywords })),
  null, 0,
).replace(/\},\{/g, '},\n  {').replace(/^\[/, '[\n  ').replace(/\]$/, ',\n]')};

/** Workspace flags. Chrome, not card illustrations — kept out of the picker. */
export const FLAGS: Illustration[] = ${JSON.stringify(
  flagRecords.map(({ hex, file, name }) => ({ hex, file, name, group: 'flags', keywords: [] })),
  null, 2,
)};
`;
...
console.log(
  ...
  `LICENSE.txt, and src/data/openmojiCatalog.ts — ${illustrationRecords.length} illustrations ` +
  `across ${usedGroups.length} groups, plus ${flagRecords.length} flags.`,
);
